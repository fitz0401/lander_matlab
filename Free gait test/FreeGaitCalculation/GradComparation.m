...
%A=reshape(t_ACD,length(t_ACD(:,1)),length(t_ACD(1,:)));
%包含的字母表示下一步该腿运动时身体不能进入这一区域，如OABCD即为不满足稳定裕度，身体不能处于的区域