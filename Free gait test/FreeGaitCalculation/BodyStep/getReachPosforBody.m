% 根据工作空间，获得可行解
%若工作空间非矩形区域，该算法是否可用？
function [BODY_FOR_CALU, Bit_BodyStableArea, Bit_ReachableArea, map] = getReachPosforBody(BODY_FOR_CALU, index, bodyMovement,BODY_PARA)



F_G(1,:)=BODY_FOR_CALU.Body(1:4);
F_G(2,:)=BODY_FOR_CALU.Body(6:9);
F_G(3,:)=BODY_FOR_CALU.Body(11:14);
...
for i = 1:4
    ...
    footMat = [BODY_FOR_CALU.Body0(i);BODY_FOR_CALU.Body0(i+5);BODY_FOR_CALU.Body0(i+10)] ...
        - [BODY_FOR_CALU.Body0(5);BODY_FOR_CALU.Body0(10);BODY_FOR_CALU.Body0(15)]; % footMat表示的是初始位置时足端相对机身质心的坐标
    ...
    delta = round(delta / BODY_FOR_CALU.Inteval) * BODY_FOR_CALU.Inteval;
...
%对Body_up进行处理，根据StepForBody计算出身体运动的最大高度，然后通过Body_up和Body_down进行筛选，得到用逻辑值表示的可行域；
[map_x,map_y,Bit_ReachableArea]=walkBody_positionWS(BODY_FOR_CALU.TBG,BODY_FOR_CALU.TCB,F_G,[BODY_FOR_CALU.Body(5),BODY_FOR_CALU.Body(10),BODY_FOR_CALU.Body(15)],BODY_PARA,bodyMovement); 

%寻找各腿工作空间重合的交集
% Bit_ReachableArea = getBodyAreaBit(BODY_FOR_CALU, step_x, step_y,bodyMovement);
...
%% 这一行代码导致形状改变，影响了后续的矩阵处理，需要修改
% Bit_ReachableArea = permute(Bit_ReachableArea,[3 2 1]);
Bit_ReachableArea=reshape(Bit_ReachableArea,1,length(Bit_ReachableArea(:,1)),length(Bit_ReachableArea(1,:)));
...
% %这里实际上只进行了对地图的欧拉角旋转，在修改后这一步并不需要
% [~, row, col] = size(map_x);
...
BODY_FOR_CALU.Body = repmat(BODY_FOR_CALU.Body,1,length(map_x(:,1)),length(map_y(1,:)));
% 若走G，则走完以后得到现在身体所在三角形。1 * BODY_FOR_CALU.len_x * BODY_FOR_CALU.len_y 的string array

map_x=reshape(map_x,1,length(map_x(:,1)),length(map_x(1,:)));
map_y=reshape(map_y,1,length(map_y(:,1)),length(map_y(1,:)));
% map_z=reshape(map_z,1,length(map_z(1,:)),length(map_z(:,1)));
%在进入这一步之前，要把所有的非可达区域置为nan
