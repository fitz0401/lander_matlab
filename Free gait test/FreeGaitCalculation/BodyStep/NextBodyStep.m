...
function [StepForBody, Foot_index, DeadLock, TargetReach] = NextBodyStep(BODY_FOR_CALU, End_Pos, Foot_index, alpha, FlagDeadLock,BODY_PARA)
...
% [map_x,map_y,Body_up,Body_down]=walkBody_positionWS(BODY_FOR_CALU.TBG,BODY_FOR_CALU.TCB,F_G,[BODY_FOR_CALU.Body(5),BODY_FOR_CALU.Body(10),BODY_FOR_CALU.Body(15)],BODY_PARA); 
% 根据下一步迈的腿，确定身体的可行域，这一部分被Body工作空间的求解算法替代，后者更加精确，尤其是在机身姿态变化较大时能更好的反应运动范围，缺点是运算较慢
% 可行域也是在平行于腿的局部坐标系的身体下的坐标系

[BODY_FOR_CALU, Bit_BodyStableArea, Bit_ReachableArea, map] = getReachPosforBody(BODY_FOR_CALU, Foot_index, StepForBody,BODY_PARA);
map_x = map(1,:,:);
map_y = map(2,:,:);
% map_z = map(3,:,:);
...
for i=1:length(map_x(1,1,:))
    A(:,i)=map_x(1,:,i);
end
for i=1:length(map_y(1,1,:))
    B(:,i)=map_y(1,:,i);
end
