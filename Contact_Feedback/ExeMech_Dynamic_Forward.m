...
% load("Adams仿真数据\TAO_res.mat")
load("样机实测数据\test01.mat")
...
%% 迭代求解
% 结果存储
v_Rv1U1_res = zeros(6,1000);
v_Rv2U2_res = zeros(6,1000);
v_Rv3U3_res = zeros(6,1000);
v_U1S1_res = zeros(6,1000);
v_U2S2_res = zeros(6,1000);
v_U3S3_res = zeros(6,1000);
a_U1S1_res = zeros(6,1000);
a_U2S2_res = zeros(6,1000);
a_U3S3_res = zeros(6,1000);
Q_int = zeros(3,1000);
tao_res = zeros(3,1000);
tao_diff = zeros(3,1000);

%for i = 1:1000
for i = 1 : total_time - (ignore_time + window_length)
    %% 各构件质心位置正解
    % 已知输入运动（足端位置：通过规划结果获得；电机位置和速度：通过编码器信息获得）
%     xF = footx(i,2);
%     yF = footy(i,2);
%     zF = footz(i,2);
%     q_a = [-0.3 -0.1 -0.3 0 0 0]';
%     q_v = [q_a(1)*i/1000 q_a(2)*i/1000 q_a(3)*i/1000 0 0 0]';
    xF = contact_data(ignore_time + window_length + i,1);
    yF = contact_data(ignore_time + window_length + i,2);
    zF = contact_data(ignore_time + window_length + i,3);
    ...
    tao = inv(Jtrans * JMotor) * average_contact_data(i,4:6)';
    q_v = [Jtrans * [average_contact_data(i,1)/1000; average_contact_data(i,2); average_contact_data(i,3)];0;0;0];
    q_a = [0 0 0 0 0 0]';
    ...
    %% 将重力和惯性力换算到驱动关节
    tao_res(:,i) = - (J_Pc1' * Q_Rv1U1 + J_Pc2' * Q_Rv2U2 + J_Pc3' * Q_Rv3U3) ... 
                   - (J_Lc1' * Q_U1S1 + J_Lc2' * Q_U2S2 + J_Lc3' * Q_U3S3);
    % tao = [taoalpha1(i,2); taoalpha2(i,2); -taoalpha3(i,2)];
    Q_int(:,i) = -(inv(J_L(4:6,:)))' * (tao + J_Pc1' * Q_Rv1U1 + J_Pc2' * Q_Rv2U2 + J_Pc3' * Q_Rv3U3 ...
                                            + J_Lc1' * Q_U1S1 + J_Lc2' * Q_U2S2 + J_Lc3' * Q_U3S3);
    tao_diff(:,i) = tao - tao_res(:,i);
end
toc

%% 结果显示
% t = linspace(0,1,1000);
i = ignore_time + window_length + 1 : total_time;
% End_F
figure(1)
subplot(3,1,1)
plot(t,Q_int(1,:))
